...
public class OperationActivity extends AppCompatActivity implements Observer {
    ...
    private void showReturnOptionsDialog() {
        android.app.AlertDialog.Builder builder = new android.app.AlertDialog.Builder(this);
        builder.setTitle("选择操作");
        builder.setMessage("请选择要执行的操作：");
        
        builder.setPositiveButton("返回主页", (dialog, which) -> {
            android.util.Log.d("OperationActivity", "用户选择返回主页");
            // 安全地返回到MainActivity
            android.content.Intent intent = new android.content.Intent(this, com.clj.blesample.MainActivity.class);
            intent.addFlags(android.content.Intent.FLAG_ACTIVITY_CLEAR_TOP | android.content.Intent.FLAG_ACTIVITY_SINGLE_TOP);
            startActivity(intent);
            finish();
        });
        
        builder.setNegativeButton("后台运行", (dialog, which) -> {
            android.util.Log.d("OperationActivity", "用户选择后台运行");
            // 移到后台继续运行
            moveTaskToBack(true);
        });
