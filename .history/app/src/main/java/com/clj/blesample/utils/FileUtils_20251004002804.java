    ...
    private static File createDataFolder() {
        try {
            ...
            if (externalDir != null && externalDir.exists()) {
                ...
                if (!folder.exists()) {
                    if (folder.mkdirs()) {
                        Log.d(TAG, "创建外部存储文件夹: " + folder.getAbsolutePath());
                        return folder;
                    }
                } else {
                    return folder;
            ...
            // 如果外部存储不可用，使用内部存储
            File internalDir = new File(Environment.getDataDirectory(), FOLDER_NAME);
            if (!internalDir.exists()) {
                if (internalDir.mkdirs()) {
                    Log.d(TAG, "创建内部存储文件夹: " + internalDir.getAbsolutePath());
                    return internalDir;
                }
            } else {
                return internalDir;
