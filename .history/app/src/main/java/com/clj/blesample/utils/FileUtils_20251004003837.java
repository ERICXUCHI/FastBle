...
public class FileUtils {
    ...
    public static void saveDataToFile(Context context, String data, String deviceName, String characteristicUuid) {
        ...
        try {
            ...
            Log.d(TAG, "应用内部目录: " + internalDir.getAbsolutePath());
            Log.d(TAG, "内部目录存在: " + internalDir.exists());
            Log.d(TAG, "内部目录可写: " + internalDir.canWrite());
            
            // 创建子文件夹
            File dataDir = new File(internalDir, FOLDER_NAME);
            if (!dataDir.exists()) {
                boolean created = dataDir.mkdirs();
                Log.d(TAG, "创建数据文件夹: " + created);
            }
            
            // 生成文件名
            String fileName = "BLE_Data.txt";  // 使用简单文件名
