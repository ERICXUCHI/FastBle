...
public class FileUtils {
     ...
     * @return 会话文件路径
     */
    public static String startNewSession(Context context, String deviceMac) {
        try {
            // 生成会话时间戳
            String sessionTime = new SimpleDateFormat("yyyyMMdd_HHmmss", Locale.getDefault()).format(new Date());
            ...
            // 创建会话文件，包含设备MAC地址
            String fileName = "BLE_Session_" + sessionTime + "_" + deviceMac.replace(":", "") + ".txt";
            String sessionFile = new File(dataDir, fileName).getAbsolutePath();
            
            // 写入会话开始信息
            FileWriter writer = new FileWriter(sessionFile);
            String sessionStart = String.format("=== BLE数据收集会话开始 ===\n");
            sessionStart += String.format("设备MAC: %s\n", deviceMac);
            ...
            // 为该设备标记会话为活跃状态
            deviceSessionTimes.put(deviceMac, sessionTime);
            deviceSessionFiles.put(deviceMac, sessionFile);
            deviceSessionActive.put(deviceMac, true);
            
            Log.d(TAG, "设备 " + deviceMac + " 新会话开始: " + sessionFile);
            return sessionFile;
     ...
     */
    public static void saveDataToFile(Context context, String data, String deviceName, String characteristicUuid, String deviceMac) {
        Log.d(TAG, "保存数据到会话文件");
        Log.d(TAG, "数据: " + data);
        ...
        // 检查该设备是否有活跃的会话
        boolean hasActiveSession = deviceSessionActive.containsKey(deviceMac) && deviceSessionActive.get(deviceMac);
        String sessionFile = deviceSessionFiles.get(deviceMac);
        
        // 如果没有活跃会话，跳过保存
        if (!hasActiveSession) {
            Log.d(TAG, "设备 " + deviceMac + " 无活跃会话，跳过数据保存");
            return;
        }
        
        // 如果会话文件不存在，使用备选方案
        if (sessionFile == null) {
            Log.e(TAG, "设备 " + deviceMac + " 无法找到会话文件，使用SharedPreferences备选方案");
        ...
        try {
            ...
            Log.d(TAG, "设备 " + deviceMac + " 数据已追加到会话文件: " + sessionFile);
            Log.d(TAG, "文件大小: " + new File(sessionFile).length() + " bytes");
     ...
     * @return 会话文件路径
     */
    public static String endCurrentSession(String deviceMac) {
        String sessionFile = deviceSessionFiles.get(deviceMac);
        if (sessionFile != null) {
            try {
                // 写入会话结束信息
                ...
                sessionEnd += String.format("设备MAC: %s\n", deviceMac);
                ...
                Log.d(TAG, "设备 " + deviceMac + " 会话结束: " + sessionFile);
                
                // 清除该设备的会话信息
                deviceSessionFiles.remove(deviceMac);
                deviceSessionTimes.remove(deviceMac);
                deviceSessionActive.remove(deviceMac);
                
                return sessionFile;
                
            } catch (Exception e) {
                Log.e(TAG, "结束设备 " + deviceMac + " 会话失败: " + e.getMessage());
            }
        }
        return null;
    }
     ...
     * 获取指定设备的会话文件路径
     * @param deviceMac 设备MAC地址
     * @return 该设备的会话文件路径，如果没有则返回null
     */
    public static String getCurrentSessionFile(String deviceMac) {
        return deviceSessionFiles.get(deviceMac);
    }
