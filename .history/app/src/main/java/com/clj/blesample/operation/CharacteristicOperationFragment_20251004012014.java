                    ...
                    btn.setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View view) {
                            if (btn.getText().toString().equals(getActivity().getString(R.string.open_notification))) {
                                ...
                                // 使用全局回调管理器创建Notify回调
                                com.clj.blesample.utils.BleCallbackManager callbackManager = 
                                    com.clj.blesample.utils.BleCallbackManager.getInstance();
                                callbackManager.init(getActivity());
                                
                                BleNotifyCallback notifyCallback = callbackManager.createNotifyCallback(
                                    bleDevice,
                                    characteristic.getService().getUuid().toString(),
                                    characteristic.getUuid().toString()
                                );
                                ...
                                BleManager.getInstance().notify(
                                        ...
                                        notifyCallback);
                                            ...
                                            public void onCharacteristicChanged(byte[] data) {
                                                ...
                                                // 保存数据到文件（使用Application Context，不依赖Fragment）
                                                android.content.Context appContext = getActivity().getApplicationContext();
                                                FileUtils.saveDataToFile(appContext, hexData, deviceName, characteristicUuid);
                                                
                                                // 记录日志
                                                android.util.Log.d("BLE_DATA", "收到数据: " + hexData + " 设备: " + deviceName);
                                                android.util.Log.e("BLE_FORCE_LOG", "强制日志 - 数据: " + hexData);
                                                
                                                // 只有在Fragment还活跃时才更新UI
