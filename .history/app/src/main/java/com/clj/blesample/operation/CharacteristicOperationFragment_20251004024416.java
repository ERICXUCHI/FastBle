...
public class CharacteristicOperationFragment extends Fragment {
    ...
    // 使用Map来为每个设备维护独立的状态
    private static Map<String, Boolean> deviceNotifyStatus = new HashMap<>();
    private static Map<String, Boolean> deviceIndicateStatus = new HashMap<>();
    
    /**
     * 获取当前设备的通知状态
     */
    private boolean isNotifyActive() {
        String deviceKey = bleDevice.getMac();
        return deviceNotifyStatus.containsKey(deviceKey) && deviceNotifyStatus.get(deviceKey);
    }
    
    /**
     * 获取当前设备的指示状态
     */
    private boolean isCurrentDeviceIndicateActive() {
        String deviceKey = bleDevice.getMac();
        return deviceIndicateStatus.containsKey(deviceKey) && deviceIndicateStatus.get(deviceKey);
    }
    
    /**
     * 设置当前设备的通知状态
     */
    private void setNotifyActive(boolean active) {
        String deviceKey = bleDevice.getMac();
        deviceNotifyStatus.put(deviceKey, active);
    }
    
    /**
     * 设置当前设备的指示状态
     */
    private void setIndicateActive(boolean active) {
        String deviceKey = bleDevice.getMac();
        deviceIndicateStatus.put(deviceKey, active);
    }
        ...
        if (childList.contains(child)) {
            ...
            switch (charaProp) {
                    ...
                    btn.setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View view) {
                            if (!isNotifyActive()) {
                                ...
                                // 开始新的数据收集会话
                                String sessionFile = com.clj.blesample.utils.FileUtils.startNewSession(getActivity(), bleDevice.getMac());
                                if (sessionFile != null) {
                                ...
                                BleManager.getInstance().notify(
                                        ...
                                        new BleNotifyCallback() {
                                            ...
                                            public void onCharacteristicChanged(byte[] data) {
                                                ...
                                                android.util.Log.d("BLE_DEBUG", "当前通知状态 - isNotifyActive: " + isNotifyActive() + ", isIndicateActive: " + isCurrentDeviceIndicateActive());
                                                try {
                                                ...
                                                android.util.Log.d("BLE_DEBUG", "Fragment状态 - isAdded: " + isAdded() + ", getActivity: " + (getActivity() != null));
                                ...
                                // 结束当前数据收集会话
                                String endedFile = com.clj.blesample.utils.FileUtils.endCurrentSession(bleDevice.getMac());
                                if (endedFile != null) {
                ...
                case PROPERTY_INDICATE: {
                    ...
                    final Button btn = (Button) view_add.findViewById(R.id.btn);
                    // 根据当前设备状态设置按钮文本
                    if (isCurrentDeviceIndicateActive()) {
                        btn.setText(getActivity().getString(R.string.close_notification));
                    } else {
                        btn.setText(getActivity().getString(R.string.open_notification));
                    }
    ...
    /**
     * 获取通知状态
     * @return 是否有任何设备的通知在运行
     */
    public static boolean isNotificationActive() {
        // 检查是否有任何设备的通知或指示在运行
        for (Boolean status : deviceNotifyStatus.values()) {
            if (status) return true;
        }
        for (Boolean status : deviceIndicateStatus.values()) {
            if (status) return true;
        }
        return false;
    }
